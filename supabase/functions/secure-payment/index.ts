  ...
  try {
    ...
    if (action === "create_checkout") {
      ...
      if (plan === "lifetime") {
        ...
        return new Response(JSON.stringify({ success: true, order_id: orderData.id, currency: currency }), {
        ...
        const orderData = await response.json();
        if (!response.ok) {
          throw new Error(orderData.error ? orderData.error.description : "Failed to create International Razorpay order");
        }
        ...
        // Domestic India Recurring Subscriptions via Razorpay Subscriptions
        const planId = plan === "monthly"
          ? "plan_SsjtsdD4GW4rR7" // SoundEngg Pro Monthly Pass (₹199/mo)
          : "plan_SsjusefppDE8Lb";  // SoundEngg Pro Yearly Pass (₹1,999/yr)
        ...
        return new Response(JSON.stringify({ success: true, subscription_id: subData.id, currency: "INR" }), {
          headers: { ...corsHeaders, "Content-Type": "application/json" }
        });
      }
    }
    ...
    if (action === "verify_payment") {
      // --- RAZORPAY VERIFICATION (HMAC SIGNATURE COMPUTATION) ---
