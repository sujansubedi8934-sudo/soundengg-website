...
function initProfessionalRTA() {
    ...
    // Frequency Labels Generator: Full 31-bands on Web/Desktop; on Native App: 31 bands in Fullscreen, 11 clean octave anchors in Normal view
    function renderFrequencyLabels() {
        if (!labelsContainer) return;
        const isNative = typeof window !== 'undefined' && (window.Capacitor?.isNativePlatform?.() || (window.Capacitor !== undefined && window.Capacitor.platform !== 'web'));
        const isWideScreen = window.innerWidth >= 1024;

        labelsContainer.innerHTML = ISO_FREQS.map((f, i) => {
            let isMajor;
            if (!isNative || isWideScreen) {
                // Web Browser / Desktop: Full 31 ISO bands always
                isMajor = (i % 2 === 0);
            } else {
                // Native Mobile App: Full 31 bands in fullscreen, 11 octave anchors in compact normal view
                isMajor = isFullscreenActive ? (i % 2 === 0) : ((i % 6 === 0) || (i === ISO_FREQS.length - 1));
            }
